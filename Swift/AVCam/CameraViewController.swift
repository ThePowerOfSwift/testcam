...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
		...
		*/
		...
		if success {
                // MARK: do something with saved video here
        }
		else {
			cleanup()
		}
    ...
    // TODO: Modify starts here
    @IBOutlet weak var imageView: UIImageView!
    let targetFrames = 10
    var currentFrame = 0
    var testCapture = false
    var urls: [URL] = []
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputSampleBuffer sampleBuffer: CMSampleBuffer!, from connection: AVCaptureConnection!) {
        ...
        if testCapture {
            ...
            if currentFrame >= targetFrames {
                ...
                // create the gif
                print("should create gif now with urls")
                
                guard let gifURL = createGif(with: urls, frameDelay: 0.5) else {
            ...
            // Save frame
            if let url = saveFrame(from: sampleBuffer) {
                urls.append(url)
            }
        }
    }
